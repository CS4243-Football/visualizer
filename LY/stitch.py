...
def mouseEvent(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONUP:
        ...
        if x>=frameX1.shape[1]:
            ...
            pts1.append((x,y))
        image   = drawMatchedKeypoints(frameX1, frameX2, pts1, pts2)
...
#main    
#vidName0    = "football_left.mov"
vidName1    = "football_mid.mov"
vidName2    = "football_right.mov"
#frameX0     = getFrame(vidName0, 5, 0.5)
frameX1     = getFrame(vidName1, 5, 0.5)
frameX2     = getFrame(vidName2, 5, 0.5)
pts1,pts2   = [],[]     # points to store matched features. Note they must be of the same size and ordering
image       = drawMatchedKeypoints(frameX1, frameX2, pts1, pts2)
...
while True:
    ...
    if key == ord("s"):
        ...
        image       = stitch(frameX1,frameX2,pts1,pts2)
    # press "f" to show automatically matched features
    if key == ord("f"):
        reset()
        pts1,pts2   = getMatchedPointsFromImages(frameX1, frameX2)
        image       = drawMatchedKeypoints(frameX1, frameX2, pts1, pts2)
